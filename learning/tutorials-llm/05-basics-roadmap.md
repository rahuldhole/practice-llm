...
|---|---|---|
| Element-wise Add | $C_{ij} = A_{ij} + B_{ij}$ | [matrix.py](../../0-foundations/20-basics/src/matrix.py#L38-L47) |
| Matrix Multiply | $C_{ij} = \sum_k A_{ik} B_{kj}$ | [matrix.py](../../0-foundations/20-basics/src/matrix.py#L60-L75) |
| Transpose | $A^T_{ji} = A_{ij}$ | [matrix.py](../../0-foundations/20-basics/src/matrix.py#L77-L83) |
...
See [nn.py](../../0-foundations/20-basics/src/nn.py) to inspect how we build a full neural network library and solve the XOR problem from scratch!
...
Check out our custom autograd engine implementation in [autograd.py](../../0-foundations/20-basics/src/autograd.py).
...
See [tokenizer.py](../../0-foundations/20-basics/src/tokenizer.py) for the complete Char and BPE implementations from scratch!
...
Check out [sequence.py](../../0-foundations/20-basics/src/sequence.py) to see our custom character prediction RNN in action!
...
Read our implementation of Single Head and Multi-Head Attention in [attention.py](../../0-foundations/20-basics/src/attention.py).
...
Check out our modular Decoder-only Tiny GPT model in [gpt.py](../../0-foundations/20-basics/src/gpt.py).
...
* **model.py**: The model is a scaled-up version of our [TinyGPT](../../0-foundations/20-basics/src/gpt.py#L33) featuring dropout layers, weight tying, and initialization adjustments.
